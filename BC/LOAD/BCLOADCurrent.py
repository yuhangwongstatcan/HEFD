...
# read directory
for x in os.walk('./BC/LOAD/files/'):
    # read files
    print(x)
    for file in x[2]:

        df = pd.read_excel(
            './BC/LOAD/files/' + file, skiprows=2, header=None, parse_dates=[0]
        ...
        df.to_csv("./BC/LOAD/output/" + filename + ".csv", index=False)