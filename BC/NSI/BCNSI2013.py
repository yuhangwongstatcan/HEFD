...
debug = True
url = "https://www.bchydro.com/content/dam/BCHydro/customer-portal/documents/corporate/suppliers/transmission-system/actual_flow_data/historical_data/jandec2013hrlytielinedata.xls"

df = pd.read_excel(
    url, skiprows=3, header=None, parse_dates=[0]
...
for (date, time) in zip(df["TIME_PERIOD"], df["HOUR"]):
    if(time != "2*"):
        ...
        if date_time > datetime(2013, 3, 10) and date_time < datetime(2013, 11, 3):
