...
debug = True
url = "https://www.bchydro.com/content/dam/BCHydro/customer-portal/documents/corporate/suppliers/transmission-system/actual_flow_data/historical_data/HourlyTielineData2017.xls"
...
# =8 Mar =7 Nov =8
df[(df['TIME_PERIOD'] == "2017-03-12") & (df['HOUR'] == 3)].drop
findDate = df[(df['TIME_PERIOD'] == "2017-11-05") & (df['HOUR'] == 2)]
print(df['TIME_PERIOD'])
print(findDate)
findIndex = findDate['HOUR'].index.values[0]
df.loc[findIndex:findIndex,'HOUR'] = 3 # change day hour.
print("******: " + str(df.loc[(df['TIME_PERIOD'] == "2017-11-05") & (df['HOUR'] == '2')]))
...
for (date, time) in zip(df["TIME_PERIOD"], df["HOUR"]):
    if(time != "2*"):
        ...
        if date_time > datetime(2017, 3, 12) and date_time < datetime(2017, 11, 5):
