...
for (date, time) in zip(df["TIME_PERIOD"], df["HOUR"]):
    if(time != "2*"):
        ...
        if date_time > datetime(2021, 3, 14) and date_time < datetime(2021, 11, 7):
            local_offset.append(7)
            daylight_offset.append(1)
        elif date_time <= datetime(2021, 3, 14) or date_time >= datetime(2021, 11, 7):
