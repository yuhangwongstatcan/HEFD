...
debug = True
# url = "https://www.bchydro.com/content/dam/BCHydro/customer-portal/documents/corporate/suppliers/transmission-system/actual_flow_data/historical_data/HourlyTielineData2020.xls"
url = "./NSI2020BC.xlsx"
df = pd.read_excel(
    url, skiprows=2, header=None, parse_dates=[0]
)  # read in the file to a pandas dataframe
# convert date format column
...
# =8 Mar =7 Nov =8
findDate = df[(df['TIME_PERIOD'] == "2020-11-01") & (df['HOUR'] == 2)]
if (findDate.empty):
    findDate = df[(df['TIME_PERIOD'] == "11/01/2020") & (df['HOUR'] == 2)]
print(df['TIME_PERIOD'])
print(findDate)
findIndex = findDate['HOUR'].index.values[0]
df.loc[findIndex:findIndex,'HOUR'] = 1 # change day hour.
# print("******: " + str(df.loc[(df['TIME_PERIOD'] == "2020-11-01") & (df['HOUR'] == '2')]))
...
for (date, time) in zip(df["TIME_PERIOD"], df["HOUR"]):
    if(time != "2*"):
        ...
        if date_time > datetime(2020, 3, 8) and date_time < datetime(2020, 11, 1):
            local_offset.append(7)
            daylight_offset.append(1)
        elif date_time <= datetime(2020, 3, 8) or date_time >= datetime(2020, 11, 1):
