...
debug = True
url = "https://www.bchydro.com/content/dam/BCHydro/customer-portal/documents/corporate/suppliers/transmission-system/actual_flow_data/historical_data/jandec2010hrlytielinedata.xls"
...
for (date, time) in zip(df["TIME_PERIOD"], df["HOUR"]):
    if(time != "2*"):
        ...
        if date_time > datetime(2010, 3, 14) and date_time < datetime(2010, 11, 7):
            local_offset.append(7)
            daylight_offset.append(1)
        elif date_time <= datetime(2010, 3, 14) or date_time >= datetime(2010, 11, 7):
