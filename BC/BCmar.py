...
debug = True
# url = "https://www.bchydro.com/content/dam/BCHydro/customer-portal/documents/corporate/suppliers/transmission-system/balancing_authority_load_data/HourlyBALoad.xls"

historical_url = [
    "https://www.bchydro.com/content/dam/BCHydro/customer-portal/documents/corporate/suppliers/transmission-system/balancing_authority_load_data/Historical%20Transmission%20Data/BalancingAuthorityLoad2023.xls"
]

for url in historical_url:
    df = pd.read_excel(
        url, skiprows=4, header=None, parse_dates=[0]
    ...
    findDate = df[(df['TIME_PERIOD'] == "2023-11-05") & (df['HOUR'] == 2)]
    findIndex = findDate['HOUR'].index.values[0]
    df.loc[findIndex:findIndex,'HOUR'] = 1
    print("******: " + str(df.loc[(df['TIME_PERIOD'] == "2023-11-05") & (df['HOUR'] == '2')]))
    ...
    for (date, time) in zip(df["TIME_PERIOD"], df["HOUR"]):
        if(time != "2*"):
            ...
            if str(date_time) != "2023-03-12 02:00:00" :
